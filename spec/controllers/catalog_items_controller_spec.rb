  ...
  describe 'GET #new' do
    ...
    context 'with search query' do
      it 'filters products by name' do
        get :new, params: { catalog_code: catalog.code, q: 'Product 1' }, format: :html
      ...
      it 'filters products by SKU' do
        get :new, params: { catalog_code: catalog.code, q: 'PROD-002' }, format: :html
    ...
    context 'with product type filter' do
      ...
      it 'filters by product type' do
        get :new, params: { catalog_code: catalog.code, product_type: 'bundle' }, format: :html
    ...
    context 'with status filter' do
      ...
      it 'filters by product status' do
        get :new, params: { catalog_code: catalog.code, status: 'draft' }, format: :html
  ...
  describe 'DELETE #destroy' do
    ...
    context 'with valid product ID' do
      it 'removes product from catalog' do
        expect {
          delete :destroy, params: { catalog_code: catalog.code, id: product1.id }, format: :html
        }.to change(catalog.catalog_items, :count).by(-1)
    ...
    context 'with product not in catalog' do
      it 'returns error' do
        delete :destroy, params: { catalog_code: catalog.code, id: product2.id }, format: :html
        expect(response).to redirect_to(catalog_items_path(catalog))
    ...
    context 'with invalid product ID' do
      it 'returns error' do
        delete :destroy, params: { catalog_code: catalog.code, id: 999999 }, format: :html
        expect(response).to redirect_to(catalog_items_path(catalog))
