...
RSpec.describe Products::ImagesComponent, type: :component do
  ...
  it "renders an Upload label bound to the hidden multi-file input" do
    ...
    expect(page).to have_css("#product_images_card[class~='group/upload']")
    expect(page).to have_css("label[for='file-upload'][class*='group-has-[#file-upload:focus-visible]/upload:ring-2']")
  end
