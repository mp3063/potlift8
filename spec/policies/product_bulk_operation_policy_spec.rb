...
RSpec.describe ProductBulkOperationPolicy do
  ...
  # Use product as a stand-in record
  let(:record) { product }
  ...
  describe "#update_labels?" do
    it "allows admins (has write scope)" do
      expect(described_class.new(admin_ctx, record).update_labels?).to be true
    end

    it "allows members (has write scope)" do
      expect(described_class.new(member_ctx, record).update_labels?).to be true
    end

    it "denies viewers (no write scope)" do
      expect(described_class.new(viewer_ctx, record).update_labels?).to be false
    end
  end

  describe "#labels_for_products?" do
    it "allows admins" do
      expect(described_class.new(admin_ctx, record).labels_for_products?).to be true
    end

    it "allows members" do
      expect(described_class.new(member_ctx, record).labels_for_products?).to be true
    end

    it "allows viewers" do
      expect(described_class.new(viewer_ctx, record).labels_for_products?).to be true
    end
  end
end
